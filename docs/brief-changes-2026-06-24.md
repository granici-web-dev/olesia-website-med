...
- **`docs/Certificat 000833 J.O.pdf`** — **WHO Basic Emergency Care Provider Course**, cert. BMP 000833, Jalba Olesea, **09.07.2026**, issued by the WHO Regional Office for Europe via the WHO Country Office in Moldova.
  ⚠️ **NEVER publish the certificate scan, its number, or the WHO logo.** The document states the Recipient agrees not to use it *"and/or the Recipient's participation … for any promotional, publicity or commercial purposes"*, and that it implies no WHO approval. A commercial practice site is exactly that use.
  🔎 **Corrected 2026-08-04 — the premise of the old note was wrong.** A WHO mention is **already live** on /about: *"Cursuri de urgențe pediatrice ale Organizației Mondiale a Sănătății"*. It is **the client's own wording**, copied verbatim from `docs/despre.md:125` — we neither invented nor added it, so there was never a line to "add after her consent". What actually needs her decision is whether it **stays**, and that is now asked properly in `questions_v3.md` §4.2 with three options (keep as is · narrow to "Curs WHO Basic Emergency Care Provider, 2026" · remove). Also flagged to her: her line says *"cursuri"* (plural, no year) while the evidence is a single 2026 course. Her words are not edited without her say-so. The restriction on the scan and the logo is not up for discussion and is recorded in a code comment at the line itself, plus a warning in the back-office "Calificări" editor where a scan would most plausibly be uploaded. The repo is private, so the PDF sitting in `docs/` is not publication.
