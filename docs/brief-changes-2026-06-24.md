...
- ~~**Real Calendly event-type API URIs** for `nutrition_copii` / `nutrition_adulti`~~ — ✅ captured 2026-08-04 from the live API (see the table below). They are the _test_ account's; the client's paid account still has to replace them, but that swap is now a **pick from a list, not a retyping**: the service form has a Calendly event picker that fills the URI and the scheduling URL together, and the Services page shows whether the account is connected and how many bookable services are still unmapped.
- **Calendly mapping RE-audited against the live API (2026-08-04)** — supersedes the 2026-07-01 "normalization", which renamed slugs to _look_ self-documenting without checking them against the account and left two services pointing at the wrong event. Read from `GET /appointments/calendly/event-types`:
  ...
  **Two mappings were wrong and are fixed:** `pediatric` pointed at the _children's-nutrition_ event type (a booking would have been filed under the wrong service), and its scheduling link used a slug that does not exist on the account at all; `integrative` pointed at the _free call_. Corrected in `seed.ts` + `apps/frontend/lib/calendly.ts`, and cross-checked programmatically — URI and scheduling URL now resolve to the same event for every service.

  ⚠ **The test account's slugs do not match their own event names** (clone artifact). The URI and the event _name/duration_ are authoritative; the slug is evidence of nothing.
...
- **`docs/Certificat 000833 J.O.pdf`** — **WHO Basic Emergency Care Provider Course**, cert. BMP 000833, Jalba Olesea, **09.07.2026**, issued by the WHO Regional Office for Europe via the WHO Country Office in Moldova.
  ⚠️ **NEVER publish the certificate scan, its number, or the WHO logo.** The document states the Recipient agrees not to use it _"and/or the Recipient's participation … for any promotional, publicity or commercial purposes"_, and that it implies no WHO approval. A commercial practice site is exactly that use.
  🔎 **Corrected 2026-08-04 — the premise of the old note was wrong.** A WHO mention is **already live** on /about: _"Cursuri de urgențe pediatrice ale Organizației Mondiale a Sănătății"_. It is **the client's own wording**, copied verbatim from `docs/despre.md:125` — we neither invented nor added it, so there was never a line to "add after her consent". What actually needs her decision is whether it **stays**, and that is now asked properly in `questions_v3.md` §4.2 with three options (keep as is · narrow to "Curs WHO Basic Emergency Care Provider, 2026" · remove). Also flagged to her: her line says _"cursuri"_ (plural, no year) while the evidence is a single 2026 course. Her words are not edited without her say-so. The restriction on the scan and the logo is not up for discussion and is recorded in a code comment at the line itself, plus a warning in the back-office "Calificări" editor where a scan would most plausibly be uploaded. The repo is private, so the PDF sitting in `docs/` is not publication.
...
- **Patient document upload** (§11.14) — ✅ **DONE 2026-08-04**, option A as recommended. No accounts: an expiring, opaque token in a URL, scoped to one appointment (or one group-C order), granting exactly _see your files / add one / remove one_. Built as one primitive so the order flow reuses it. Back office issues, copies, emails, revokes, downloads and erases from the appointment sheet; the public page is at `/{locale}/incarcare/{token}`, `noindex` + `no-referrer` so the token never leaks in a Referer header.
  ...
  - **Automatic purge** (daily cron) deletes rows _and_ bytes past the retention period.
...
| `nutrition_copii` _(new)_  | —                          | 60 min · 38 EUR                                 |
| `nutrition_adulti` _(new)_ | —                          | 60 min · 38 EUR                                 |
...
- **Transcript:** `whisper-cli` (large-v3-turbo, `-l ro -ml 40 -sow`) over the audio of the 139 MB original. Hand-corrected: _Olesia→Olesea_, _medic-pediatru→medic pediatru_, _minținerea→menținerea_, _profesionalii→profesionalism_. EN/RU are faithful translations on the same timings.
...
**Contradiction on record:** `docs/questions.md` answers 17–18 said _"подтверждение вручную в портале"_ and _"онлайн-оплату — нет"_; `module_calendly.md` states **"Payments: out of scope — manual"**; Prisma carries `paymentStatus: pending → confirmed` set by hand; decision #3 of 2026-06-24 made paid downloads manual for the same reason. Answer v2 §11 now requires **pay online before the slot is confirmed**.
...
**📎 Reference the client sent (2026-07-26): `docs/telegram-cloud-photo-size-2-5400158683477515871-y.jpg`** — a competitor's "ask the doctor" checkout (_"Dr. Petrache va raspunde"_, general medicine + diabetes, **answer within max 30 minutes**, price per single question, "after payment follow the instructions"). What it actually shows:
...
- [x] **Dependency updates** — ✅ DONE 2026-07-26: `.github/dependabot.yml`.
  ...
  - ✅ **CI landed 2026-07-28** (`.github/workflows/ci.yml`), which is what makes these PRs mergeable on sight. Two jobs: **verify** (frozen-lockfile install → build shared types → generate the Prisma client → typecheck the API → build all three apps) and **migrations** (apply every migration to a throwaway Postgres 16, then `migrate diff --exit-code` to prove they add up to `schema.prisma`).
    - The API needed **five pre-existing type errors fixed first** — a CI that is red on `main` from day one teaches everyone to ignore it. Unused import, a self-referential inference in the Calendly pager, two `Json`-column casts in `about.service`, and a `paths` entry for `sharp` (it ships `lib/index.d.ts` but its `exports` map has no `types` condition, so `moduleResolution: nodenext` silently typed the import as `any`).
    - The API is bundled by webpack with SWC, which does **not** typecheck — hence the explicit `tsc --noEmit` step. The frontend and back-office builds typecheck as they go.
    - ⚠️ It proves the code compiles, **not that anything works**: there are still no tests in this repo. Green means "not obviously broken", not "approved".
