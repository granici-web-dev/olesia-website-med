...
const CHOOSE_WHEN: Bi[] = [
  {
    ro: 'Vrei sprijin constant, nu o vizită singulară.',
    en: 'You want steady support, not a one-off visit.',
    ru: 'Вам нужна постоянная поддержка, а не разовый визит.',
  },
  {
    ro: 'Lucrezi la un obiectiv de durată — alimentație, creștere, un plan.',
    en: 'You’re working toward a longer goal — diet, growth, a plan.',
    ru: 'Вы движетесь к долгосрочной цели — питание, рост, план.',
  },
  {
    ro: 'Ai nevoie de ajustări pe parcurs, pe măsură ce lucrurile se schimbă.',
    en: 'You need adjustments along the way, as things change.',
    ru: 'Вам нужно корректировать план по ходу, когда меняется ситуация.',
  },
  {
    ro: 'Vrei să poți întreba medicul între consultații.',
    en: 'You want to be able to ask the doctor between consultations.',
    ru: 'Вы хотите спрашивать врача между консультациями.',
  },
];

const INCLUDES: Bi[] = [
  {
    ro: 'Monitorizare periodică (greutate, creștere, alimentație, analize)',
    en: 'Periodic monitoring (weight, growth, diet, lab results)',
    ru: 'Периодическое наблюдение (вес, рост, питание, анализы)',
  },
  {
    ro: 'Ajustarea planului medical sau alimentar pe parcurs',
    en: 'Adjusting the medical or nutrition plan over time',
    ru: 'Корректировка медицинского или пищевого плана',
  },
  {
    ro: 'Comunicare directă cu medicul (email/WhatsApp)',
    en: 'Direct communication with the doctor (email/WhatsApp)',
    ru: 'Прямая связь с врачом (email/WhatsApp)',
  },
  {
    ro: 'Prioritate la programarea consultațiilor',
    en: 'Priority when booking consultations',
    ru: 'Приоритет при записи на консультации',
  },
];
...
const TYPES: { name: Bi; note: Bi }[] = [
  {
    name: {
      ro: 'Abonament Pediatrie',
      en: 'Pediatrics subscription',
      ru: 'Абонемент «Педиатрия»',
    },
    note: {
      ro: 'Sănătatea și dezvoltarea copilului',
      en: 'Child health and development',
      ru: 'Здоровье и развитие ребёнка',
    },
  },
  {
    name: {
      ro: 'Abonament Nutriție copii',
      en: 'Child nutrition subscription',
      ru: 'Абонемент «Питание детей»',
    },
    note: {
      ro: 'Alimentație și creștere pentru copii',
      en: 'Feeding and growth for children',
      ru: 'Питание и рост для детей',
    },
  },
  {
    name: {
      ro: 'Abonament Nutriție adulți',
      en: 'Adult nutrition subscription',
      ru: 'Абонемент «Питание взрослых»',
    },
    note: {
      ro: 'Obiective de nutriție pentru adulți',
      en: 'Nutrition goals for adults',
      ru: 'Цели по питанию для взрослых',
    },
  },
  {
    name: {
      ro: 'Abonament Complex (Pediatrie + Nutriție)',
      en: 'Complex subscription (Pediatrics + Nutrition)',
      ru: 'Комплексный абонемент (педиатрия + питание)',
    },
    note: {
      ro: 'Abordare integrată, sănătate și alimentație împreună',
      en: 'Integrated approach — health and nutrition together',
      ru: 'Комплексный подход: здоровье и питание вместе',
    },
  },
];
...
const FAQ: { q: Bi; a: Bi }[] = [
  {
    q: {
      ro: 'Ce tipuri de abonament există?',
      en: 'What subscription types are there?',
      ru: 'Какие бывают типы абонемента?',
    },
    a: {
  ...
  {
    q: {
      ro: 'Ce durată pot alege?',
      en: 'What durations can I choose?',
      ru: 'Какую длительность можно выбрать?',
    },
    a: {
  ...
  },
  {
    q: {
      ...
      ru: 'Это для детей или также для взрослых?',
    },
    a: {
    ...
    <main className="bg-cream text-ink">
      ...
      <section className="border-b border-[var(--rule)]">
        <div className="shell py-20 md:py-28">
          <p className="mb-10 inline-flex items-center gap-2.5 text-[11px] font-medium uppercase tracking-[0.16em] text-ink-soft">
            ...
            />
            {ru
              ? 'Абонементы · Онлайн-портал'
              : en
                ? 'Subscriptions · Online portal'
                : 'Abonamente · Portal online'}
          </p>
          <div className="grid items-start gap-10 md:grid-cols-[1.15fr_0.85fr] md:gap-14 lg:gap-20">
            <div>
              <h1 className="serif max-w-[15ch] text-[clamp(2.6rem,6vw,5.4rem)] leading-[1.04] tracking-[-0.015em] text-balance">
                {ru ? (
                  <>
                    Наблюдение и{' '}
                    <span className="serif-it text-sage">абонементы</span>
                  </>
                ) : en ? (
                  <>
                    Monitoring &amp;{' '}
                    <span className="serif-it text-sage">subscriptions</span>
                  </>
                ) : (
                  <>
                    Monitorizare și{' '}
                    <span className="serif-it text-sage">abonamente</span>
                  </>
                )}
              </h1>
            ...
            <div className="md:border-l md:border-[var(--rule)] md:pl-12 lg:pl-16">
              <p className="mono inline-flex items-center rounded-full border border-[var(--rule)] px-3.5 py-1.5 text-[11px] uppercase tracking-[0.12em] text-ink-soft">
                {ru
                  ? 'Онлайн-портал · 1–6 месяцев'
                  : en
                    ? 'Online portal · 1–6 months'
                    : 'Portal online · 1–6 luni'}
              </p>
        ...
        <div className="shell grid gap-10 py-16 md:grid-cols-[1fr_1.3fr] md:gap-20 md:py-24">
          <div className="md:sticky md:top-[133px] md:self-start">
            <p className="eyebrow mb-3">
              {ru ? 'Абонементы' : en ? 'Subscriptions' : 'Abonamente'}
            </p>
        ...
        <header className="max-w-[40rem]">
          ...
          <h2 className="serif text-[clamp(2.1rem,3.8vw,3.4rem)] leading-[1.04] tracking-[-0.02em] text-pretty">
            {ru ? (
              ...
              <>
                Pentru cine e{' '}
                <span className="serif-it text-sage">potrivită</span>
              </>
            )}
          </h2>
        </header>
      ...
      <section className="bg-paper">
        <div className="shell grid gap-10 py-16 md:grid-cols-[1fr_1.3fr] md:gap-20 md:py-24">
          <div>
            <p className="eyebrow mb-3">
              {ru ? 'В программе' : en ? 'In the program' : 'În program'}
            </p>
            <h2 className="serif text-[clamp(1.9rem,3.4vw,2.8rem)] leading-[1.05] tracking-[-0.02em] text-balance">
              {ru ? (
                <>
        ...
        <div className="shell grid gap-10 py-16 md:grid-cols-2 md:gap-20 md:py-20">
          ...
          <div>
            ...
            <p className="max-w-[56ch] leading-relaxed text-ink-soft text-pretty">
              {ru ? (
                <>
                  Сопровождение не подходит для неотложных ситуаций и не
                  заменяет лечение диагностированного заболевания. Если ситуация
                  срочная, звоните{' '}
                  <span className="font-medium text-ink">112</span> или
                  обратитесь в ближайшую службу неотложной помощи.
                </>
              ) : en ? (
                <>
                  Monitoring isn’t for medical emergencies and doesn’t replace
                  treatment for a diagnosed condition. If the situation is
                  urgent, call <span className="font-medium text-ink">112</span>{' '}
                  or go to the nearest emergency service.
                </>
              ) : (
                <>
                  Acompanierea nu este pentru urgențe medicale și nu înlocuiește
                  tratamentul unei afecțiuni diagnosticate. Dacă situația e
                  urgentă, sună la{' '}
              ...
              <ul className="grid gap-3 text-[1.05rem]">
                <li>
                  <Link href="/integrative" className={creamUnderline}>
                    {ru
                      ? 'Разовая глубокая оценка? → Интегративная консультация'
                      : en
                        ? 'A one-off deep assessment? → Integrative consultation'
                        : 'O evaluare aprofundată unică? → Consultație integrativă'}
                  </Link>
                </li>
                <li>
