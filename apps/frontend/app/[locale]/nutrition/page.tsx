...
const HELP_CHILDREN: Bi[] = [
  {
    ro: 'Dificultăți de hrănire și refuzul mâncării',
    en: 'Feeding difficulties and food refusal',
    ru: 'Трудности с кормлением и отказ от еды',
  },
  {
    ro: 'Refuzul alimentației, inclusiv refuzul biberonului',
    en: 'Food refusal, including bottle aversion',
    ru: 'Отказ от еды, в том числе отказ от бутылочки',
  },
  {
    ro: 'Introducerea diversificării (trecerea la alimente solide)',
    en: 'Starting solids (the move to solid foods)',
    ru: 'Введение прикорма (переход к твёрдой пище)',
  },
  {
    ro: 'Copilul mofturos la mâncare',
    en: 'The picky eater',
    ru: 'Привередливый в еде ребёнок',
  },
  {
    ro: 'Alimentația pentru o creștere sănătoasă',
    en: 'Eating for healthy growth',
    ru: 'Питание для здорового роста',
  },
];

const HELP_ADULTS: Bi[] = [
  {
    ro: 'Plan alimentar personalizat',
    en: 'A personalized nutrition plan',
    ru: 'Индивидуальный план питания',
  },
  {
    ro: 'Greutate și obiceiuri alimentare sănătoase',
    en: 'Weight and healthy eating habits',
    ru: 'Вес и здоровые пищевые привычки',
  },
  {
    ro: 'Alimentație echilibrată, bazată pe dovezi',
    en: 'Balanced, evidence-based eating',
    ru: 'Сбалансированное питание, основанное на доказательствах',
  },
];
...
const FOCUS: Bi[] = [
  ...
  {
    ro: 'Dificultăți de hrănire',
    en: 'Feeding difficulties',
    ru: 'Трудности с кормлением',
  },
  { ro: 'Diversificare', en: 'Starting solids', ru: 'Введение прикорма' },
  {
    ro: 'Plan alimentar pentru adulți',
    en: 'Adult nutrition plans',
    ru: 'Планы питания для взрослых',
  },
];
...
const FAQ: { q: Bi; a: Bi }[] = [
  ...
  {
    q: {
      ro: 'Primesc un plan scris?',
      en: 'Do I get a written plan?',
      ru: 'Будет ли письменный план?',
    },
    a: {
...
export default async function NutritionPage({
  ...
  const copiiLabel = ru
    ...
    : en
      ? 'Book · children'
      : 'Programează · copii';
  const adultiLabel = ru
    ? 'Записаться · взрослые'
    : en
      ? 'Book · adults'
      : 'Programează · adulți';
  const copiiReason = ru
    ? 'Консультация по питанию · дети'
    : en
      ? 'Nutrition consultation · children'
      : 'Consultație de nutriție · copii';
  const adultiReason = ru
    ? 'Консультация по питанию · взрослые'
    : en
      ? 'Nutrition consultation · adults'
      : 'Consultație de nutriție · adulți';
  ...
  const HelpGroup = ({ title, items }: { title: string; items: Bi[] }) => (
    <Reveal className="border-t border-[var(--rule)] pt-7">
      <h3 className="serif text-[1.7rem] leading-snug tracking-[-0.01em]">
        {title}
      </h3>
    ...
    <main className="bg-cream text-ink">
      ...
      <section className="border-b border-[var(--rule)]">
        <div className="shell py-20 md:py-28">
          <p className="mb-10 inline-flex items-center gap-2.5 text-[11px] font-medium uppercase tracking-[0.16em] text-ink-soft">
            ...
            />
            {ru
              ? 'Питание · Видеоконсультация'
              : en
                ? 'Nutrition · Video consultation'
                : 'Nutriție · Consultație video'}
          </p>
              ...
              <h1 className="serif max-w-[15ch] text-[clamp(2.6rem,6vw,5.4rem)] leading-[1.04] tracking-[-0.015em] text-balance">
                {ru ? (
                  <>
                    ...
                    <span className="serif-it text-sage">питанию</span>
                  </>
                ) : en ? (
                  <>
                    Online <span className="serif-it text-sage">nutrition</span>{' '}
                    consultations
                  </>
                ) : (
                  <>
      ...
      <section className="shell py-20 md:py-28">
        ...
        <div className="mt-12 grid gap-x-16 gap-y-12 md:grid-cols-2">
          <HelpGroup
            title={ru ? 'Для детей' : en ? 'For children' : 'Pentru copii'}
            items={HELP_CHILDREN}
          />
          <HelpGroup
            title={ru ? 'Для взрослых' : en ? 'For adults' : 'Pentru adulți'}
            items={HELP_ADULTS}
          />
        </div>
      </section>
      ...
      <section className="shell grid items-start gap-12 border-b border-[var(--rule)] py-20 md:grid-cols-[1.05fr_0.95fr] md:gap-20 md:py-28">
        <div className="md:sticky md:top-[133px] md:self-start">
          ...
          <p className="eyebrow mt-9 mb-3">
            {ru ? 'Направления' : en ? 'Focus areas' : 'Domenii de focus'}
          </p>
