...
const HELP: { label: Bi; text: Bi }[] = [
  ...
  {
    label: {
      ro: 'Probleme digestive',
      en: 'Digestive issues',
      ru: 'Проблемы с пищеварением',
    },
    text: {
  ...
  {
    label: {
      ro: 'Creștere și dezvoltare',
      en: 'Growth and development',
      ru: 'Рост и развитие',
    },
    text: {
  ...
  {
    label: {
      ro: 'Alimentație și apetit',
      en: 'Feeding and appetite',
...
const FOCUS: Bi[] = [
  {
    ro: 'Nutriție și dificultăți de hrănire',
    en: 'Nutrition and feeding difficulties',
    ru: 'Питание и трудности с кормлением',
  },
  {
    ro: 'Probleme digestive',
    en: 'Digestive issues',
...
const FAQ: { q: Bi; a: Bi }[] = [
  {
    q: {
      ro: 'Pot primi o rețetă în urma consultației?',
      en: 'Can I get a prescription after the consultation?',
      ru: 'Можно ли получить рецепт после консультации?',
    },
    a: {
  ...
  {
    q: {
      ro: 'În ce limbi pot discuta cu medicul?',
      en: 'Which languages can I speak with the doctor?',
      ru: 'На каких языках можно общаться с врачом?',
    },
    a: {
  ...
  {
    q: {
      ro: 'Trebuie să fie copilul prezent la apel?',
      en: 'Does my child need to be on the call?',
      ru: 'Должен ли ребёнок присутствовать на звонке?',
    },
    a: {
  ...
  },
  {
    q: {
      ...
      ru: 'Можно ли перенести или отменить запись?',
    },
    a: {
  ...
  {
    q: {
      ro: 'Ce documente să pregătesc?',
      en: 'What documents should I prepare?',
      ru: 'Какие документы подготовить?',
    },
    a: {
      ...
      <section className="border-b border-[var(--rule)]">
        <div className="shell py-20 md:py-28">
          <p className="mb-10 inline-flex items-center gap-2.5 text-[11px] font-medium uppercase tracking-[0.16em] text-ink-soft">
            ...
            />
            {ru
              ? 'Педиатрия · Видеоконсультация'
              : en
                ? 'Pediatrics · Video consultation'
                : 'Pediatrie · Consultație video'}
          </p>
            ...
            <div>
              <h1 className="serif max-w-[15ch] text-[clamp(2.6rem,6vw,5.4rem)] leading-[1.04] tracking-[-0.015em] text-balance">
                {ru ? (
                  <>
                    Онлайн{' '}
                    <span className="serif-it text-sage">педиатрические</span>{' '}
                    консультации
                  </>
                ) : en ? (
                  <>
                    Online <span className="serif-it text-sage">pediatric</span>{' '}
                    consultations
                  </>
                ) : (
                  <>
                    Consultații{' '}
                    <span className="serif-it text-sage">pediatrice</span>{' '}
                    online
                  </>
                )}
              </h1>
              <p className="mt-7 max-w-[32ch] text-[1.125rem] leading-[1.6] text-ink-soft text-pretty">
                {ru
                  ? 'Здоровье ребёнка — не выходя из дома.'
                  : en
                    ? "For your child's health, from home."
                    : 'Pentru sănătatea copilului tău, de acasă.'}
              </p>
      ...
      <section className="shell py-20 md:py-28">
        <header className="max-w-[46rem]">
          <p className="eyebrow mb-3">
            {ru
              ...
              : en
                ? 'Reasons to book'
                : 'Motive să programezi'}
          </p>
          <h2 className="serif text-[clamp(2.1rem,3.8vw,3.4rem)] leading-[1.04] tracking-[-0.02em] text-pretty">
            {ru ? (
              <>
                С чем поможет{' '}
                <span className="serif-it text-sage">педиатрическая</span>{' '}
                консультация
              </>
            ) : en ? (
              <>
                What a pediatric consultation{' '}
                <span className="serif-it text-sage">helps with</span>
              </>
            ) : (
              <>
                Cu ce te poate <span className="serif-it text-sage">ajuta</span>{' '}
                o consultație pediatrică
              </>
            )}
          </h2>
        </header>
        <div className="mt-12 grid gap-x-12 gap-y-9 sm:grid-cols-2 lg:grid-cols-3">
          {HELP.map((it, i) => (
            ...
            >
              <h3 className="serif text-[1.4rem] leading-snug tracking-[-0.01em]">
                {lc(it.label)}
              </h3>
        ...
        <div className="md:sticky md:top-[133px] md:self-start">
          ...
          <h2 className="serif text-[clamp(1.9rem,3.6vw,3rem)] leading-[1.06] tracking-[-0.02em] text-balance">
            {ru ? (
              <>
                Кто вас{' '}
                <span className="serif-it text-sage">консультирует</span>
              </>
            ) : en ? (
              <>
        ...
        <div className="shell grid gap-10 py-16 md:grid-cols-2 md:gap-20 md:py-20">
          ...
          <div>
            ...
            <p className="max-w-[56ch] leading-relaxed text-ink-soft text-pretty">
              {ru ? (
                <>
                  Онлайн-консультация не предназначена для неотложных случаев.
                  Если состояние ребёнка тяжёлое или быстро ухудшается, звоните{' '}
                  <span className="font-medium text-ink">112</span> или
                  обратитесь в ближайшую службу скорой помощи. Иногда нужен
                  очный осмотр или дополнительные обследования — в этом случае
                  мы понятно всё объясним и подскажем.
                </>
              ) : en ? (
                <>
                  An online consultation isn’t meant for medical emergencies. If
                  your child’s condition is serious or worsening fast, call{' '}
                  <span className="font-medium text-ink">112</span> or go to the
                  nearest emergency service. Some situations may need a physical
                  exam or further tests — we’ll tell you clearly and guide you.
                </>
              ) : (
                <>
                  Consultația video nu este destinată urgențelor medicale. Dacă
                  starea copilului este gravă sau se agravează rapid, sună la{' '}
                  <span className="font-medium text-ink">112</span> sau mergi la
                  cel mai apropiat serviciu de urgență. Unele situații pot
                  necesita o examinare fizică sau investigații suplimentare — în
                  acest caz îți spunem clar și te îndrumăm.
                </>
              )}
            </p>
          </div>
              ...
              <ul className="grid gap-3 text-[1.05rem]">
                <li>
                  <Link href="/services#nutrition" className={creamUnderline}>
                    {ru
                      ? 'Проблемы с питанием? → Консультация по питанию'
                      : en
                        ? 'Feeding problems? → Nutrition consultation'
