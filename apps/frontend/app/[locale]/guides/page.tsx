...
export default async function LibraryPage({
  ...
  const HOW = [
    {
      title: {
        ro: 'Alege un material',
        en: 'Pick a material',
        ru: 'Выберите материал',
      },
      text: {
        ro: 'Caută și filtrează după vârstă și categorie.',
        en: 'Search and filter by age and category.',
        ru: 'Ищите и фильтруйте по возрасту и категории.',
      },
    },
    {
      title: {
        ro: 'Descarcă sau cumpără',
        en: 'Download or buy',
    ...
    {
      title: {
        ro: 'Citește în ritmul tău',
        en: 'Read at your pace',
        ru: 'Читайте в своём ритме',
      },
      text: {
        ro: 'Salvează materialul și revino oricând ai nevoie.',
        en: 'Save the material and come back whenever you need it.',
        ru: 'Сохраните материал и возвращайтесь, когда понадобится.',
      },
    },
  ];
  ...
  return (
    <main className="bg-cream text-ink">
            ...
            <div>
              <h1 className="serif max-w-[15ch] text-[clamp(2.6rem,6vw,5.4rem)] leading-[1.03] tracking-[-0.015em] text-balance">
                {ru ? (
                  <>
                    Цифровая{' '}
                    <span className="serif-it text-sage">библиотека</span>
                  </>
                ) : en ? (
                  <>
                    The digital{' '}
                    <span className="serif-it text-sage">library</span>
                  </>
                ) : (
                  <>
                    Biblioteca{' '}
                    <span className="serif-it text-sage">digitală</span>
                  </>
                )}
              </h1>
            ...
            {featured && (
              <div className="md:border-l md:border-[var(--rule)] md:pl-12 lg:pl-16">
                <p className="eyebrow mb-5">
                  {ru ? 'Рекомендуем' : en ? 'Featured' : 'Recomandat'}
                </p>
                <article className="flex flex-col border border-[var(--rule)] bg-paper">
                  ...
                  <div className="p-6">
                    ...
                    <a href="#library" className={`mt-6 ${cardCta}`}>
                      {ru
        ...
        <header className="mb-10 flex flex-col gap-4 md:flex-row md:items-baseline md:justify-between">
          <div>
            <p className="eyebrow mb-3">
              {ru ? 'Библиотека' : en ? 'Library' : 'Bibliotecă'}
            </p>
            <h2 className="serif text-[clamp(2.2rem,4.5vw,4rem)] leading-[1.0] tracking-[-0.02em] text-balance">
              {ru ? (
                ...
                <>
                  Găsește un{' '}
                  <span className="serif-it text-sage">material</span>
                </>
              )}
            </h2>
          </div>
        ...
        <div className="shell py-20 md:py-24">
          ...
          <h2 className="serif text-[clamp(2rem,4vw,3.2rem)] leading-[1.02] tracking-[-0.02em] text-cream text-balance">
            {ru ? (
              <>
                В пару{' '}
                <span className="serif-it text-[var(--sage-soft)]">кликов</span>
              </>
            ) : en ? (
              <>
                In a few{' '}
                <span className="serif-it text-[var(--sage-soft)]">taps</span>
              </>
            ) : (
              <>
                În câțiva{' '}
                <span className="serif-it text-[var(--sage-soft)]">pași</span>
              </>
            )}
          </h2>
          ...
          <div className="relative mt-14 grid gap-x-8 gap-y-12 md:mt-16 md:grid-cols-3">
            ...
            {HOW.map((step, i) => (
              <div key={step.title.en} className="relative">
                ...
                <h3 className="serif text-[1.5rem] leading-snug text-cream">
                  {lc(step.title)}
                </h3>
      ...
      <section className="shell grid gap-12 border-b border-[var(--rule)] py-20 md:grid-cols-2 md:gap-20 md:py-24">
        <div>
          <p className="eyebrow mb-3">
            {ru ? 'Кто их пишет' : en ? 'Who writes them' : 'Cine le scrie'}
          </p>
          <h2 className="serif text-[clamp(1.8rem,3.2vw,2.6rem)] leading-[1.05] tracking-[-0.02em] text-balance">
            {ru ? (
              ...
              <>
                Written by a{' '}
                <span className="serif-it text-sage">pediatrician</span>
              </>
            ) : (
              <>
                Scrise de un medic{' '}
                <span className="serif-it text-sage">pediatru</span>
              </>
            )}
          </h2>
        ...
        <div className="md:border-l md:border-[var(--rule)] md:pl-12 lg:pl-16">
          <p className="eyebrow mb-3">
            {ru ? 'Важно знать' : en ? 'Good to know' : 'De reținut'}
          </p>
