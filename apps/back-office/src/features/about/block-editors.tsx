...
export function TestimonialsEditor({ form }: { form: Form }) {
  const { fields, append, remove, move } = useBlock(form, 'testimonials');
  const t = a.testimonials;
  return (
    <BlockSection
      title={t.title}
      hint={t.hint}
        ...
        >
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <Field
              ...
              {...form.register(`testimonials.${i}.author`)}
            />
            <Field
              label={t.roleRo}
              placeholder={t.rolePlaceholderRo}
              {...form.register(`testimonials.${i}.roleRo`)}
            />
            <Field
              label={t.roleEn}
...
export function FaqEditor({ form }: { form: Form }) {
  const { fields, append, remove, move } = useBlock(form, 'faq');
  const q = a.faq;
  return (
    <BlockSection
      title={q.title}
